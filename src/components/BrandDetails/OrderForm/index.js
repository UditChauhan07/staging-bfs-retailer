    ...
    if (!showTable) {
        return (
            <div>
                ...
                <div>{errorOnlist > 0 && <p className='text-start mt-2 text-danger'>Product list has some issue.</p>}</div>
                {isLoading && <><table className='table table-hover text-start'>
                    <thead>
                        <tr>
                            <th>Title</th>
                            <th>Product Code</th>
                    ...
                    <tbody>
                        {data.map((item, index) => {
                            ...
                            if (item?.Quantity) {
                                ...
                                return (
                                    <tr key={index}>
                                        <td style={error ? { background: '#FFC1C3',color:'#fff' } : {}}>{productDetails?.Name || '---'}</td>
                                        <td style={error ? { background: '#FFC1C3',color:'#fff' } : {}}>{productDetails?.ProductCode || item['Product Code']}</td>
                                        <td style={error ? { background: '#FFC1C3',color:'#fff' } : {}}>{productDetails?.Category__c || 'No Category'}</td>
                                        <td style={error ? { background: '#FFC1C3',color:'#fff' } : {}}>{productDetails?.ProductUPC__c || item['ProductUPC']}</td>
                                        <td style={error ? { background: '#FFC1C3',color:'#fff' } : {}}>{productDetails?.usdRetail__c || '---'}</td>
                                        <td style={error ? { background: '#FFC1C3',color:'#fff' } : {}}>${productDetails?.salesPrice || '---'}</td>
                                        <td style={error ? { background: '#FFC1C3',color:'#fff' } : {}}>{productDetails?.Min_Order_QTY__c || 0}</td>
                                        ...
                                        <td style={error ? { background: '#FFC1C3',color:'#fff' } : {}}>{item?.Quantity || 0}</td>
                                    </tr>)
                            }
                        })}
                        {(errorOnlist && data.length>0 && errorOnlist==data.length) &&<tr>
                    ...
                    <div className="d-flex justify-content-center">
                        ...
                        <button style={{ color: '#000', fontFamily: 'Montserrat', fontSize: '14px', fontStyle: 'normal', fontWeight: 600, lineHeight: 'normal', letterSpacing: '1.4px', width: '100px', height: '30px', backgroundColor: '#fff', border: '1px solid #000', margin: '0 2rem', textTransform: 'uppercase' }} onClick={() => setOrderFromModal(false)}>Close</button>
                    </div></>}
            </div>
        );
    } else {
